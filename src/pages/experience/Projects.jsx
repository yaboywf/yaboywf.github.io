...
const projects = [
    ...
    { title: "Multiplayer Rock Paper Scissors with Socket.io", description: "Multiplayer game that allows users to play against each other. Uses Socket.io for real-time communication.", usage: ["React JS", "SCSS", "NodeJS (Socket.io)"], image: "rps.webp", link: "https://github.com/yaboywf/multiplayer-rock-paper-scissors"},
...
const ProjectGrid = ({ gridColumn, gridRow, data = {}, onExpand }) => {
    ...
    return (
        <div style={{ gridColumn, gridRow }} ref={ref} className="project-grid" onClick={() => onExpand(ref, data)}>
            <p>{data.title}</p>
        </div>
    )
}

const MorphingCard = ({ expanded, onFullyClose }) => {
    ...
    return (
        <div
            ref={ref}
            className="project-grid morphing-card"
            style={{
                top: rect.top + "px",
                left: rect.left + "px",
                width: rect.width + "px",
...
const Projects = () => {
    ...
    const onClose = () => {
        originalRef.current.classList.remove("hidden");
        setExpanded(null);
    };
    ...
    return (
        <section className="projects">
            ...
            <div className="overlay" style={{ opacity: expanded ? "1" : "0", pointerEvents: expanded ? "all" : "none" }}></div>

            {expanded && <MorphingCard expanded={expanded} onFullyClose={onClose} />}
        </section>
    );
};
